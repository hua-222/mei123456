<template>
	<div class="box">
		...
		<el-row class="ShaiXuan">
			...
			<el-col :span="6">
				<div><span>筛选</span>&ensp;<i class="el-icon-caret-bottom icon"></i></div>
			</el-col>
		</el-row>
		<CarList @JieShou="isCollect" :ShouChang="ShouChang" :unfold="unfold" :bodys="bodys"></CarList>
		<mt-popup v-model="popupVisible" style="top: 5.3125rem;" :modal="false" class="tishi" position="top">
...
<script>
	...
	export default {
		...
		methods: {
			...
			isCollect(id) {
				for (var i = 0; i < this.bodys.length; i++) {
					if (this.bodys[i].id == id) {
						if ((this.bodys[i].collect == false)) {
							this.bodys[i].collect = true
						} else {
							this.bodys[i].collect = false
						}
					}
				}
				var list=this.bodys;
		...
		mounted() {
			if(localStorage.cmts!=undefined){
